...
export default function Sidebar() {
  ...
  const updateShowProfile = useStoryStore(
    (state) => state.updateShowProfile
  );
  ...
  async function handleGenerate() {
    try {
      ...
      setStory({
        ...newStory,
        background: story.background || "/images/default-story.jpg",
        showProfile: story.showProfile,
      });
    } catch {
  ...
  return (
    <>
      <aside className="w-full border-b border-zinc-800 bg-zinc-950 p-6 lg:w-80 lg:border-b-0 lg:border-r">
        <h2 className="mb-6 text-lg font-semibold">Editor</h2>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-lg border border-zinc-800 bg-zinc-900/60 p-4">
            <p className="text-xs leading-5 text-zinc-400">
              O Last.fm não fornece o total de scrobbles por período. O valor
              exibido na cápsula é uma estimativa baseada nas músicas mais
              ouvidas desse período ₍^. .^₎Ⳋ.
            </p>
          </div>

          <div className="flex items-center justify-between rounded-lg border border-zinc-800 bg-zinc-900/60 p-4">
            <div>
              <p className="text-sm font-medium">Mostrar perfil</p>
              <p className="text-xs text-zinc-400">
                Exibe foto e @ do usuário na cápsula.
              </p>
            </div>

            <input
              type="checkbox"
              checked={story.showProfile}
              onChange={(event) =>
                updateShowProfile(event.target.checked)
              }
              className="h-4 w-4"
            />
          </div>
